...
class InstructorServiceSpec extends FreeSpec with PropertyChecks with Codecs {
  ...
  "POST to /instructors should create an instructor" in {
    forAll { instructorForm: InstructorForm =>
      val (response, created) =
        withClient { client =>
          for {
            (response, created) <- client.fetch(createRequest.withEntity(instructorForm.asJson))(
                                    res => res.as[Instructor[Instructor.Id]].tupleLeft(res))
          ...
          for {
            creationAttempts <- instructorForms
                                 .traverse(
                                   i =>
                                     client
                                       .fetch(createRequest.withEntity(i.asJson))(res =>
          ...
          withClient { client =>
            for {
              created <- client.fetchAs[Instructor[Instructor.Id]](
                          createRequest.withEntity(instructorForm.asJson))
        ...
        withClient { client =>
          for {
            ...
            retrievedPreDelete <- client.status(
                                   getRequest.withUri(
                                     Uri.unsafeFromString(s"/${created.id.value}")))
            _ <- client.status(deleteRequest.withUri(Uri.unsafeFromString(s"/${created.id.value}")))
  ...
  private val createRequest = Request[IO](method = Method.POST)
  private val getRequest = Request[IO]()
  private val patchRequest = Request[IO](method = Method.PATCH)
  private val deleteRequest = Request[IO](method = Method.DELETE)
